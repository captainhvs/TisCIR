...
from models import Phi,CLIPAttention
...
@torch.no_grad()
def extract_image_features(dataset: Dataset, clip_model: CLIPVisionModelWithProjection, batch_size: Optional[int] = 64,
...
def extract_pseudo_tokens_with_phi(clip_model: CLIPVisionModelWithProjection, phi: Phi, dataset: Dataset, args) -> Tuple[torch.Tensor, List[str]]:
    ...
    """
    data_loader = DataLoader(dataset=dataset, batch_size=64, num_workers=1, pin_memory=False,
...
@torch.no_grad()
def extract_pseudo_tokens_without_msmp(clip_model: CLIPVisionModelWithProjection,  dataset: Dataset, args) -> Tuple[List[str]]:
    """
    Extracts pseudo tokens from a dataset using a CLIP model and a phi model
    """
    data_loader = DataLoader(dataset=dataset, batch_size=64, num_workers=1, pin_memory=False,
...
@torch.no_grad()
def extract_embeddings_with_attention(clip_model: CLIPVisionModelWithProjection, attention: CLIPAttention, dataset: Dataset, args) -> Tuple[torch.Tensor, List[str]]:
    """
    Extracts pseudo tokens from a dataset using a CLIP model and an attention model
    """
    data_loader = DataLoader(dataset=dataset, batch_size=64, num_workers=10, pin_memory=False,
                             collate_fn=collate_fn,persistent_workers=True)
    predicted_tokens = []
    names_list = []
    print(f"Extracting tokens using Attention model")
    for batch in tqdm(data_loader):
        images = batch.get('image')
        names = batch.get('image_name')
        if images is None:
        ...
        images = images.to(device)
        image_features = clip_model(pixel_values=images.half()).image_embeds
        image_features += 1.0 * torch.rand(image_features.shape[0], device=image_features.device).unsqueeze(
            -1) * torch.randn(image_features.shape, device=image_features.device)
        if args.l2_normalize:
            image_features = F.normalize(image_features, dim=-1)

        batch_predicted_tokens = attention(image_features)
        # batch_predicted_tokens = image_features
        if args.l2_normalize:
            batch_predicted_tokens  = F.normalize(batch_predicted_tokens, dim=-1)
...
@torch.no_grad()
def extract_embeddings_with_attention_visualize(clip_model: CLIPVisionModelWithProjection, attention: CLIPAttention, dataset: Dataset, args) -> Tuple[torch.Tensor, List[str]]:
    """
    Extracts pseudo tokens from a dataset using a CLIP model and an attention model
    """
    data_loader = DataLoader(dataset=dataset, batch_size=64, num_workers=10, pin_memory=False,
    ...
    for batch in tqdm(data_loader):
        ...
        predicted_tokens.append(batch_predicted_tokens.cpu())
        image_embedding.append(image_features.cpu())
        names_list.extend(names)
        original_image.append(images)

    # predicted_tokens = torch.vstack(predicted_tokens)
    return predicted_tokens,image_embedding,original_image,names_list
