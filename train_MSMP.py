...
from loader import build_loader_with_id, CIRRDataset
...
def parse_args():
    ...
    parser.add_argument("--output_dir", default="./train_out", type=str,
    ...
    parser.add_argument("--position_id",default=1, type=int, help="Which position to replace with phi.")
...
def save_phi(name: str, cur_epoch: int, Position_id: int, model_to_save: Phi, training_path: Path) -> None:
    """
    Save the weights of Phi during training
    """
    models_path = os.path.join(training_path, "checkpoints_MSMP_"+str(Position_id))
...
def train_phi(args):
    ...
    train_dataset = build_loader_with_id(args, tokenizer, accelerator,args.position_id)
            ...
            if accelerator.sync_gradients:
                ...
                if args.checkpointing_steps and global_step % args.checkpointing_steps == 0:
                    if accelerator.is_main_process:
                        ...
                        save_phi(f"phi_{global_step:09}", global_step,args.position_id, accelerator.unwrap_model(phi), args.output_dir)
                        save_phi(f"phi_latest", global_step, args.position_id, accelerator.unwrap_model(phi), args.output_dir)
                    if args.use_ema:
                        phi_for_saving = copy.deepcopy(accelerator.unwrap_model(phi))
                        ema_phi.copy_to(phi_for_saving.parameters())
                        save_phi(f"ema_phi_{global_step:09}", global_step, args.position_id, phi_for_saving, args.output_dir)
                        save_phi(f"ema_phi_latest", global_step, args.position_id, phi_for_saving, args.output_dir)
                ...
                if args.validation_steps and (global_step % args.validation_steps == 0 or global_step == 50):
                    if accelerator.is_main_process:
                        ...
                        similarity = cirr_compute_val_text(cirr_relative_val_dataset, text_encoder,
                                                                     cirr_val_index_features, cirr_val_index_names,
                                                                     cirr_val_ref_names_list, cirr_val_pseudo_tokens)


                        accelerator.log({f"validate": similarity}, step=global_step)
                        # print(json.dumps(similarity, indent=1))
                        ...
                        if args.checkpointing_steps:
                            if similarity > best_sim:
                                ...
                                save_phi("phi_best", global_step, args.position_id, accelerator.unwrap_model(phi), args.output_dir)
